...
def count_breed_recursive(dogs, breed):
    if not dogs:
        ...
        first_dog = dogs[0]
        rest = dogs[1:]
        if first_dog.breed == breed:
            return 1 + count_breed_recursive(rest, breed)
        else:
            return count_breed_recursive(rest, breed)
...
def count_color_recursive(dogs, color):
    if not dogs:
        ...
        first_dog = dogs[0]
        rest = dogs[1:]
        if first_dog.color == color:
            return 1 + count_color_recursive(rest, color)
        else:
            return count_color_recursive(rest, color)
...
def find_dog_names_regex(dogs):
    lijst = []
    for dog in dogs:
        if re.match(r'^B(.)*\1', dog.name):
            lijst.append(dog.name)
    return lijst
...
def count_ancestors(dog, color):
    if not dog:
        ...
        return count_ancestors(dog.father or dog.mother, color)
